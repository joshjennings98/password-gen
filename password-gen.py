...
def generatePassword(dictionary : Dict[str, str], wordNum : int = 5, extra : int = 0) -> str:
    ...
    """
    if extra > wordNum:
        raise Exception("Insert one symbol per word at maximum.")
    
    words = []
    
    for _ in range(wordNum):
        rolls = "".join([str(rollDice()) for i in range(5)])
    ...
    for _ in range(extra):
        wordIdx = randomInt(0, wordNum-1)
        
        while (wordIdx in done.items()): # I wish python had do while loops :(
            wordIdx = randomInt(0, wordNum-1)
        done[wordIdx] = 1

        charIdx = randomInt(0, len(words[wordIdx]) - 1)

        x = rollDice()
        y = rollDice()
        extraChar = extraChars[y-1][x-1]
        
        chars = list(words[wordIdx])
        chars[charIdx] = extraChar
        words[wordIdx] = "".join(chars)
